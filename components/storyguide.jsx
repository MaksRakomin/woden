...
// ─── StoryGuide Wrappers ──────────────────────────────────────────────────────
function SgHead({ label, title }) { return <div className="mb-5"><div className="text-sm font-bold uppercase tracking-[0.12em] text-primary mb-1">{label}</div><h1 className="text-[26px] font-extrabold text-contrast leading-[1.15]">{title}</h1></div>; }
function SgBody({ children, className='' }) { return <p className={`text-[16px]  leading-[1.7] text-ink-soft mb-4 ${className}`}>{children}</p>; }
...
function SgCard({ label, children, className='' }) { return <div className={`bg-base border border-light-gray rounded-xl p-5 mb-3.5 shadow-sm ${className}`}>{label && <div className="text-sm font-bold uppercase tracking-[0.1em] text-ink-faint mb-2.5">{label}</div>}{children}</div>; }
function SgSectionH({ children }) { return <h3 className="text-base font-bold text-black mt-6 mb-3">{children}</h3>; }
function SgInfoBanner({ arcs }) { return <div className="bg-super-light-gray border border-light-gray rounded-lg px-3.5 py-2 text-xs text-ink-soft mb-4 flex flex-wrap gap-1.5 items-center"><strong className="text-contrast">StoryKernel Arc:</strong>{arcs.map(a => <span key={a} className="inline-block px-2 py-0.5 rounded-[5px] text-[14px] font-medium bg-primary-bg-subtle text-secondary">{a}</span>)}</div>; }
function SgMsgPoint({ text, personas }) { return <div className="bg-super-light-gray border border-light-gray rounded-lg p-3 mb-2"><div className="text-sm text-ink-soft leading-[1.6] mb-1.5">{text}</div>{personas?.map(p => <span key={p} className="inline-block px-2 py-0.5 rounded-[5px] text-[14px] font-medium bg-light-gray text-ink-soft mr-1">{p}</span>)}</div>; }
function SgObjection({ q, a }) { return <div className="bg-base border border-light-gray rounded-[10px] p-3.5 mb-2.5"><div className="text-sm font-semibold text-contrast mb-1.5 before:content-['\201C'] before:mr-0.5 before:text-primary">{q}</div><div className="text-sm text-ink-soft leading-[1.6]">{a}</div></div>; }
        ...
        <div className="flex h-full min-h-0 bg-super-light-gray relative">
            ...
            <aside className={`bg-base border-r border-light-gray flex flex-col overflow-hidden md:static md:translate-x-0 md:w-[240px] md:shrink-0 md:h-full fixed top-0 bottom-0 left-0 w-[280px] z-[150] transition-transform duration-200 ${navOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
                <div className="px-4 pt-4 pb-3 border-b border-light-gray shrink-0 relative">
                    <div className="text-sm font-bold text-contrast truncate mb-1 pr-8">{EFC.client}</div>
                        ...
                        if (entry.type === 'item') {
                            ...
                            return (
                                <button key={entry.id} onClick={() => goPage(entry.id)}
                                    className={`w-full text-left flex items-center gap-2 px-3.5 py-2 text-sm font-sans truncate transition-colors
                                    ...
                                    <span className={`shrink-0 w-4 text-center text-[14px] ${page === entry.id ? 'opacity-100' : 'opacity-40'}`}>{entry.icon}</span>
                                    {entry.label}
                                </button>
                            );
                        }
                        ...
                        return (
                            <div key={entry.id}>
                                <button onClick={() => toggleGroup(entry.id)}
                                    className={`flex items-center gap-2 w-full text-left px-3.5 py-2 text-sm font-semibold font-sans transition-colors hover:bg-super-light-gray
                                        ${isActiveSection ? 'text-primary' : 'text-contrast'}`}>
                                    <span className="shrink-0 w-4 text-center text-[14px] opacity-40">{entry.icon}</span>
                                    {entry.label}
                                    <span className="ml-auto text-sm text-ink-faint shrink-0">{isOpen ? '▾' : '▸'}</span>
                                </button>
                                {isOpen && entry.items.map(item => {
                                    ...
                                    return (
                                        <button key={item.id} onClick={() => goPage(item.id)}
                                            className={`w-full text-left flex items-start gap-2 py-1.5 pr-2 text-sm font-sans transition-colors
                                                ${item.sub2 ? 'pl-10 text-[14px] text-ink-faint' : 'pl-7'}
            ...
            <main className={`flex-1 min-w-0 flex flex-col ${page === 'ai' ? 'overflow-hidden' : 'overflow-y-auto'}`} ref={mainRef}>
                <div className="md:hidden sticky top-0 z-30 bg-base border-b border-light-gray flex items-center gap-3 px-4 py-2.5 shrink-0">
                    ...
                    <div className="text-sm font-bold uppercase tracking-wider text-contrast">Sections</div>
                </div>
                {page === 'ai' ? (
            ...
            <div className="flex-1 overflow-y-auto flex flex-col" ref={bodyRef} style={{minHeight: 0}}>
                {messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center flex-1 px-4 py-8 text-center">
                        ...
                        <div className="text-lg font-extrabold text-contrast mb-2">How can I help you?</div>
                        <div className="text-sm text-ink-soft max-w-[360px] leading-[1.65] mb-8">
                        ...
                        <div className="w-full max-w-[760px] grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {suggestions.map(s => (
                                <button key={s} onClick={() => send(s)}
                                    className="text-left px-4 py-3 border border-light-gray rounded-xl bg-base font-sans text-sm text-ink-soft hover:border-primary hover:text-contrast hover:shadow-sm transition-all leading-[1.5]">
                                    {s}
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-col gap-4 py-2 pb-4 max-w-[720px] w-full mx-auto px-1">
                        {messages.map(m => (
                            <div key={m.id} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                {m.role === 'assistant' && (
                                    <div className="w-7 h-7 rounded-lg bg-contrast flex items-center justify-center text-sm shrink-0 mt-0.5">
                                        <span style={{color: '#f5c842'}}>⚡</span>
                                    </div>
                                )}
                                <div className={`max-w-[78%] px-4 py-3 rounded-2xl text-sm leading-[1.65] whitespace-pre-wrap
                                    ${m.role === 'user'
                ...
                <div className="max-w-[720px] mx-auto">
                    <div className="flex gap-2 items-end bg-base border border-light-gray rounded-2xl px-4 py-3 focus-within:border-contrast transition-colors shadow-sm">
                        <textarea rows={1} value={input} ref={inputRef}
                            ...
                            className="flex-1 resize-none min-h-[22px] max-h-[120px] text-sm font-sans bg-transparent outline-none text-contrast placeholder:text-ink-faint" />
                        <button onClick={() => send()}
                            disabled={!input.trim()}
                            className="w-8 h-8 rounded-xl bg-contrast border-none text-base cursor-pointer text-sm shrink-0 flex items-center justify-center transition-all hover:opacity-80 disabled:opacity-30 disabled:cursor-default">
                    ...
                    <div className="text-center text-[14px] text-ink-faint mt-2">
    ...
    return (
        <div className="flex flex-col gap-5">
            ...
            <div className="bg-contrast rounded-2xl px-5 py-6 sm:px-8 sm:py-7">
                <div className="text-sm uppercase tracking-[0.14em] text-primary mb-2 font-bold">EFC International</div>
                <div className="text-[26px] font-extrabold text-base mb-2.5 leading-[1.15]">Digital StoryGuide</div>
                <div className="text-sm text-white/60 leading-[1.75] max-w-[480px]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                    ...
                    <button key={id} onClick={() => goPage(id)}
                        ...
                        <div className="font-bold text-lg  text-contrast mb-1">{title}</div>
                        <div className="text-sm text-ink-faint leading-[1.6]">{desc}</div>
                    </button>
                ))}
            </div>
            ...
            <div className="bg-base border border-light-gray rounded-xl flex flex-col overflow-hidden h-[70vh] sm:h-[500px]">
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-2.5" ref={bodyRef}>
                    {messages.length === 0 && (
                        <div>
                            <div className="max-w-[85%] px-4 py-3 rounded-[18px] rounded-bl-sm text-sm leading-[1.55] bg-super-light-gray text-contrast mb-3">
                            ...
                            <div className="flex flex-col gap-1.5">
                                {window.WODEN.EFC_CHAT_SUGGESTIONS.map(s => (
                                    <button key={s} onClick={() => send(s)}
                                        className="text-left px-3.5 py-2 border border-light-gray rounded-[18px] bg-base font-sans text-sm cursor-pointer transition-colors text-ink-soft hover:bg-contrast hover:text-white hover:border-contrast">
                                        {s}
                                    </button>
                                ))}
                            </div>
                    ...
                    {messages.map(m => (
                        <div key={m.id}
                            className={`max-w-[85%] px-4 py-3 rounded-[18px] text-sm leading-[1.55] whitespace-pre-wrap
                                ${m.role === 'user'
                ...
                <div className="px-4 py-3 border-t border-light-gray flex gap-2 items-end bg-base shrink-0">
                    <textarea rows={1} value={input}
                        ...
                        onChange={e => setInput(e.target.value)}
                        onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(); } }}
                        className="flex-1 resize-none min-h-[38px] max-h-[80px] px-3.5 py-2 border border-light-gray rounded-[18px] text-sm font-sans bg-super-light-gray outline-none focus:border-contrast focus:bg-base" />
                    <button onClick={() => send()}
            ...
            {arcs.map(a => (
                <div key={a.n} className="border border-light-gray rounded-[10px] p-3.5 mb-2.5 bg-base">
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        <div className="w-[22px] h-[22px] rounded-full bg-contrast text-base text-[14px] font-bold flex items-center justify-center shrink-0">{a.n}</div>
                        <span className="font-bold text-[14px]  text-contrast">{a.title}</span>
                        <span className="inline-block px-2 py-0.5 rounded-[5px] text-sm font-medium bg-primary-bg-subtle text-secondary ml-1">{a.stage}</span>
                    </div>
                    <div className="text-sm text-ink-soft leading-[1.6]">{a.why}</div>
                </div>
            ))}
        </div>
    );
...
function PageSkFull() {
    ...
    return (
        <div>
            ...
            {arcs.map((a, i) => (
                <div key={a.n} className={`rounded-lg border p-3.5 mb-2.5 text-[16px]  text-contrast leading-[1.7] ${colors[i]}`} title={a.title}>
                    <div className="text-sm font-bold uppercase tracking-[0.08em] text-ink-faint mb-1.5">{a.title}</div>
                    {a.passage}
                </div>
            ))}
        </div>
...
function PageMhIntro({ goPage }) {
    ...
    return (
        <div>
            ...
            <div className="text-sm font-bold uppercase tracking-[0.1em] text-[#aaa] mb-2.5">Internal Elements</div>
            ...
            <SgCard label="Core Values">{m.values.map(v => <div key={v.name} className="flex gap-2.5 mb-2.5 items-start"><div className="w-2 h-2 rounded-full bg-primary mt-1.5 shrink-0" /><div className="text-sm text-ink-soft leading-[1.6]"><strong className="text-contrast">{v.name}:</strong> {v.body}</div></div>)}</SgCard>
            <div className="text-sm font-bold uppercase tracking-[0.1em] text-[#aaa] mt-5 mb-2.5">External Elements</div>
            ...
            <SgCard label="Verbal Identity"><SgBody>These are the terms EFC should own in customers' minds. Use them consistently across all channels.</SgBody>{m.verbalIdentity.map(v => <span key={v.phrase} className="inline-block px-2 py-0.5 rounded-[5px] text-[14px] font-medium bg-super-light-gray text-ink-soft border border-light-gray mr-1.5 mb-1.5">{v.phrase}</span>)}<div className="mt-3">{m.verbalIdentity.map(v => <div key={v.phrase} className="text-sm text-[#555] mb-2"><strong className="text-contrast">{v.phrase}</strong> — {v.note}</div>)}</div></SgCard>
        </div>
    );
}
...
function MhValues() { return <div><SgBody>EFC's four core values define how the team shows up — internally and in every customer interaction.</SgBody>{window.WODEN.EFC.messaging.values.map((v, i) => <SgCard key={v.name}><div className="flex items-center gap-2.5 mb-2"><div className="w-[22px] h-[22px] rounded-full bg-contrast text-base text-[14px] font-bold flex items-center justify-center shrink-0">{i+1}</div><div className="text-sm font-bold uppercase tracking-[0.1em] text-ink-faint m-0">{v.name}</div></div><SgBody className="!mb-0">{v.body}</SgBody></SgCard>)}</div>; }
...
function PageIcpFull() {
    ...
    return (
        <div>
            ...
            <SgCard label="Their Company"><div className="overflow-x-auto -mx-2"><table className="w-full min-w-[460px] border-collapse text-sm"><tbody>{icp.company.map(r => <tr key={r.label}><td className="py-2 px-2.5 border-b border-light-gray font-semibold text-contrast w-[180px] align-top">{r.label}</td><td className="py-2 px-2.5 border-b border-light-gray text-ink-soft align-top">{r.value}</td></tr>)}</tbody></table></div></SgCard>
            <SgCard label="How to Find Them"><div className="overflow-x-auto -mx-2"><table className="w-full min-w-[460px] border-collapse text-sm"><tbody>{icp.howToFind.map(r => <tr key={r.channel}><td className="py-2 px-2.5 border-b border-light-gray font-semibold text-contrast w-[180px] align-top">{r.channel}</td><td className="py-2 px-2.5 border-b border-light-gray text-ink-soft align-top">{r.detail}</td></tr>)}</tbody></table></div></SgCard>
            <SgCard label="Key Phrases from the StoryKernel">{icp.keyPhrases.map(p => <div key={p.phrase} className="border-b border-[#f0f0f0] py-2.5"><div className="font-semibold text-sm italic text-[#333] mb-0.5">"{p.phrase}"</div><div className="text-sm text-[#888]">{p.note}</div></div>)}</SgCard>
        </div>
    );
}
...
function PageCjAwareness() {
    ...
    return (
        <div>
            ...
            <div className="bg-contrast rounded-xl p-5 text-white/75 text-sm leading-[1.8] mb-5">
                <div className="text-sm uppercase tracking-[0.12em] text-white/40 mb-3 font-bold">Script</div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-5">
                {aw.ctas.map((c, i) => <div key={i} className="bg-base border border-light-gray rounded-[10px] p-3"><div className="text-sm font-bold text-contrast mb-1">{c.persona}</div><div className="text-sm text-ink-soft leading-[1.5]">{c.text}</div></div>)}
            </div>
        </div>
    );
}
...
function PageCjConsideration() {
    ...
    return (
        <div>
            ...
            <SgSectionH>Calls to Action</SgSectionH>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-5">
                {co.ctas.map((c, i) => <div key={i} className="bg-base border border-light-gray rounded-[10px] p-3"><div className="text-sm font-bold text-contrast mb-1">{c.persona}</div><div className="text-sm text-ink-soft leading-[1.5]">{c.text}</div></div>)}
            </div>
        </div>
    );
}
            ...
            {personas.map(p => (
                <div key={p.name} className="bg-base border border-light-gray rounded-xl p-5 mb-3.5">
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="min-w-0"><div className="font-bold text-[15px] text-contrast truncate">{p.name}</div><div className="text-sm text-ink-faint truncate">{p.title} · {p.industry}</div></div>
                    </div>
                    <div className="text-sm text-ink-soft mb-1.5 leading-[1.5]"><strong className="text-contrast">Challenges:</strong> {p.challenges}</div>
                    <div className="text-sm text-ink-soft mb-1.5 leading-[1.5]"><strong className="text-contrast">Triggers:</strong> {p.triggers}</div>
                    <div className="text-sm text-ink-soft mb-3.5 leading-[1.5]"><strong className="text-contrast">Goals:</strong> {p.goals}</div>
                    <div className="border-t border-[#f0f0f0] pt-3">
                        <div className="text-sm font-bold uppercase tracking-[0.1em] text-[#aaa] mb-2">Targeted Messaging</div>
                        {p.msgs.map((msg, i) => <div key={i} className="text-sm text-[#555] mb-1.5 leading-[1.6]">• {msg}</div>)}
                    </div>
                </div>
            ))}
        </div>
    ...
    return (
        <div>
            ...
            <SgCard label="Short-Form Origin Story (75–100 words)" className="mt-3.5">
                <p className="text-sm text-[#555] leading-[1.7] m-0">{os.short}</p>
            </SgCard>
        </div>
    );
}
...
function PageCjCx() {
    ...
    return (
        <div>
            ...
            <SgSectionH>Calls to Action</SgSectionH>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-5">
                {cx.ctas.map((c, i) => <div key={i} className="bg-base border border-light-gray rounded-[10px] p-3"><div className="text-sm font-bold text-contrast mb-1">{c.label}</div><div className="text-sm text-ink-soft leading-[1.5]">{c.body}</div></div>)}
            </div>
            <SgCard label="Sample Welcome Email — Subject: Getting Connected with EFC" className="mt-3.5 border-primary bg-primary-bg-subtle">
                <div className="text-sm text-[#444] leading-[1.8] whitespace-pre-line">{cx.welcomeEmail}</div>
            </SgCard>
        </div>
    );
}
        ...
        <div>
            ...
            {stories.map((s, i) => (
                <div key={i} className="bg-base border border-light-gray rounded-xl p-5 mb-4">
                    ...
                    <div className="text-sm text-ink-soft leading-[1.75] mb-2.5">{s.body}</div>
                    <span className="inline-block px-2 py-0.5 rounded-[5px] text-[14px] font-medium bg-primary-bg-subtle text-secondary">Outcome: {s.outcome}</span>
                </div>
            ))}
        </div>
    );
        ...
        <div>
            ...
            {quotes.map((q, i) => (
                <div key={i} className="bg-base border border-light-gray rounded-xl p-5 mb-3.5">
                    ...
                    <div className="text-[14px] font-bold uppercase tracking-[0.1em] text-ink-faint mb-1.5">— {q.who}</div>
                    <div className="text-sm text-ink-soft leading-[1.6]">{q.context}</div>
                </div>
            ))}
        </div>
    );
        ...
        <div>
            ...
            {comps.map((c, i) => (
                <div key={i} className="bg-base border border-light-gray rounded-xl p-5 mb-3.5">
                    ...
                    <div className="text-sm text-ink-soft leading-[1.75]">{c.body}</div>
                </div>
            ))}
        </div>
    );
    ...
    return (
        <div>
            ...
            <div className="bg-base border-[1.5px] border-dashed border-light-gray rounded-xl p-10 text-center text-ink-faint">
                ...
                <p className="text-sm">This section will be populated with {title.toLowerCase()} content.</p>
            </div>
        </div>
    );
}
