...
function Modal({ title, onClose, children }) {
  ...
  return (
      <div className="fixed inset-0 bg-black/45 z-[200] grid place-items-center" onClick={onClose}>
        <div className="bg-base border border-light-gray rounded-[24px] w-[480px] max-w-[calc(100vw-40px)] p-8 shadow-lg" onClick={e => e.stopPropagation()}>
...
function SideNav({ role, route, nav, onLogout }) {
  ...
  return (
      <aside className="w-[248px] shrink-0 bg-base border-r border-light-gray flex flex-col sticky top-0 h-screen z-20">
        <div className="p-5 border-b border-light-gray flex items-center gap-2">
          <a href="#/" onClick={(e)=>{ e.preventDefault(); nav(role ? ({admin:'/admin',manager:'/manager',client:'/client',employee:'/employee/storyguide'})[role] : '/login'); }} className="font-extrabold text-2xl tracking-tight text-contrast flex items-center gap-2">
        ...
        <nav className="flex flex-col gap-1 p-4 flex-1 overflow-y-auto">
          {items.map((item, i) => {
            ...
            return (
                <a key={path} onClick={(e)=>{ e.preventDefault(); nav(path); }} href={'#' + path}
...
function SubBar({ route, role, search, onSearch }) {
  ...
  return (
      <div className="flex items-center gap-4 px-8 py-3.5 bg-base border-b border-light-gray sticky top-0 z-10">
        <div className="flex items-center gap-2 font-mono text-xs text-ink-soft tracking-wide flex-1">
          <span>{rootLabel}</span>
          {tail.map((t, i) => (
              <React.Fragment key={i}>
                <span className="text-gray">/</span>
                <span className={i === tail.length - 1 ? 'text-contrast font-bold uppercase' : ''}>{t}</span>
              </React.Fragment>
          ))}
        ...
        {onSearch != null && (
            <div className="relative flex items-center">
              ...
              <input type="text" placeholder="Search StoryGuide..." value={search} onChange={e => onSearch(e.target.value)}
                     className="w-56 focus:w-72 transition-all pl-8 pr-3 py-1.5 border border-light-gray rounded-full bg-super-light-gray text-xs font-sans text-contrast focus:outline-none focus:border-contrast focus:bg-base focus:shadow-focus placeholder-ink-faint" />
            </div>
        )}
      </div>
  );
...
function RoleSwitcher({ role, setRole, nav, chatOpen }) {
  ...
  return (
      <div className={`fixed bottom-6 z-[100] transition-all duration-300 ${chatOpen ? 'left-6 right-auto' : 'right-6 left-auto'}`}>
        {open && (
            <div className={`absolute bottom-16 w-[340px] bg-base border border-light-gray rounded-[24px] p-4 shadow-lg ${chatOpen ? 'left-0' : 'right-0'}`}>
